...
### Front door
Signed out, `/phase1` makes the case for registering, with the catalogue of
every tool. Signed in, there is no page there: an agent is taken straight to the
dashboard and an administrator to the operations console.
...
One tool, with no separate charge: automatic refresh.
...
| Accounts, sessions, workspaces, photographs (MongoDB) | Automatic refresh runs |
