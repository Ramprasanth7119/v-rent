...
export function PublicPreview({
  ...
  return (
    <div className={cx('overflow-hidden rounded-2xl bg-p1-surface text-p1-text shadow-p1-sm ring-1 ring-p1-border', className)}>
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className={cx('font-p1display font-bold leading-[1.15] tracking-[-0.022em] text-p1-text text-balance', compact ? 'text-[19px]' : 'text-[27px]')}>{title}</h3>
        ...
        <ul className="mt-4 flex flex-wrap gap-2 text-[13.5px] text-p1-text">
          {facts.map((f) => <li key={f.v} className="inline-flex items-center gap-1.5 rounded-full bg-p1-subtle px-3 py-1.5"><f.icon size={14} className="text-p1-text-3" aria-hidden />{f.v}</li>)}
        </ul>
        ...
        <div className={cx('mt-5 grid gap-5', !compact && 'lg:grid-cols-[minmax(0,1fr)_280px]')}>
          <div className="min-w-0">
            <h4 className="font-p1display text-[17px] font-bold tracking-[-0.012em] text-p1-text">About this property</h4>
            ...
            {(l.amenities?.length ?? 0) > 0 && (
              <>
                <h4 className="mt-6 font-p1display text-[17px] font-bold tracking-[-0.012em] text-p1-text">Amenities</h4>
          ...
          <aside className="h-fit rounded-xl bg-p1-surface p-5 shadow-p1-md ring-1 ring-p1-border">
            <div className="flex items-center gap-3">
              <Avatar name={preferredName(agent.fullName) || 'Agent'} size="lg" />
              <div className="min-w-0">
                <div className="truncate font-p1display text-[16px] font-bold text-p1-text">{preferredName(agent.fullName)}</div>
