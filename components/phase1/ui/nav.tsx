        ...
        {steps.map((s, i) => {
          ...
          const inner = (
            <>
              <span className={cx('flex shrink-0 items-center justify-center rounded-full border-2 font-semibold transition-colors', compact ? 'h-6 w-6 text-[11px]' : 'h-7 w-7 text-[12px]',
                done ? 'border-p1-success bg-p1-success text-white' : active ? 'border-p1-primary bg-p1-primary text-white dark:border-p1-info dark:bg-p1-info dark:text-p1-primary-on' : 'border-p1-border-strong bg-p1-surface text-p1-text-3')}>
                {done ? <Check size={13} strokeWidth={3} aria-hidden /> : i + 1}
              </span>
              <span className="min-w-0">
                <span className={cx('block leading-[1.35]', compact ? 'text-[12.5px]' : 'text-[13.5px]', active ? 'font-semibold text-p1-text' : done ? 'font-medium text-p1-text-2' : 'text-p1-text-3')}>
                  {s.label}
          ...
          return (
            <li key={s.label} className="flex min-w-0 items-start gap-2 sm:flex-1" aria-current={active ? 'step' : undefined}>
              {clickable && s.href ? (
                <Link href={s.href} className="flex min-w-0 items-start gap-2.5 rounded-lg hover:underline underline-offset-4">{inner}</Link>
              ) : clickable && onSelect ? (
                <button type="button" onClick={() => onSelect(i)} className="flex min-w-0 items-start gap-2.5 rounded-lg text-left hover:underline underline-offset-4 cursor-pointer">{inner}</button>
              ) : (
                <span className="flex min-w-0 items-start gap-2.5">{inner}</span>
              )}
              {i < steps.length - 1 && <span className={cx('mx-2 hidden h-0.5 min-w-3 flex-1 rounded-full sm:block', compact ? 'mt-3' : 'mt-3.5', done ? 'bg-p1-success' : 'bg-p1-border')} aria-hidden />}
            </li>
          );
        })}
      </ol>
