...
export function FilterChips<T extends string>({ options, value, onChange, label = 'Filter', size = 'md' }: { options: { key: T; label: string; count?: number }[]; value: T; onChange: (k: T) => void; label?: string; size?: 'sm' | 'md' }) {
  return (
      ...
      {options.map((o) => {
        ...
        return (
          <button key={o.key} type="button" aria-pressed={on} onClick={() => onChange(o.key)}
            className={cx('inline-flex items-center gap-1.5 rounded-full border font-medium transition-colors cursor-pointer', size === 'sm' ? 'h-8 px-3 text-[12.5px]' : 'h-9 px-3.5 text-[13px]',
              on ? 'border-p1-primary bg-p1-primary text-p1-primary-on dark:border-p1-info dark:bg-p1-info' : 'border-p1-border bg-p1-surface text-p1-text-2 hover:border-p1-border-strong hover:text-p1-text')}>
            {o.label}
            {typeof o.count === 'number' && <span className={cx('rounded-full px-1.5 text-[11.5px] tabular-nums', on ? 'bg-white/20' : 'bg-p1-subtle text-p1-text-3')}>{o.count}</span>}
          </button>
        );
      })}
    </div>
    ...
    <div className={cx('overflow-auto', !flush && 'rounded-xl border border-p1-border bg-p1-surface')} style={maxHeight ? { maxHeight } : undefined}>
      <table className="w-full border-collapse text-left text-[13.5px]" style={{ minWidth }}>
        ...
        <thead className="sticky top-0 z-[1]">
          <tr className="border-b border-p1-border bg-p1-subtle/80 backdrop-blur">
            {columns.map((c) => {
              ...
              return (
                <th key={c.key} scope="col" style={{ width: c.width }} aria-sort={active ? (sort!.dir === 'asc' ? 'ascending' : 'descending') : undefined}
                  className={cx('whitespace-nowrap px-4 py-2.5 text-[12px] font-semibold text-p1-text-3', align(c.align), c.hideBelow && hide[c.hideBelow])}>
                  {sortable ? (
