...
import {
  ...
  usePagination, Pagination, EmptyState, cx } from '../../../../components/phase1/kit';
import { LISTING_STATUS } from '../../../../components/phase1/status';
import { useCountUp, useInView } from '../../../../components/phase1/hooks';
import { FUNNEL, SUBSCRIPTIONS } from '../../../../lib/phase1/data';
import { ACTION_LABEL, AuditRow, IS_ADVERSE } from '../../../../lib/phase1/audit-labels';
...
export default function ReportsView({ auditRows }: { auditRows: AuditRow[] }) {
  ...
  const audit = auditRows.filter((r) => !q
    || [ACTION_LABEL[r.action], r.actorEmail, r.subjectName, r.listingRef ?? '', r.reason ?? '']
  ...
  const columns: Column<AuditRow>[] = [
    {
      ...
      render: (r) => (
        <span className="whitespace-nowrap font-mono text-[13px] text-p1-text-2">
    ...
    },
    {
      ...
      render: (r) => (
        <span className="flex items-center gap-2">
          ...
          <span className="text-[14px] text-p1-text">{ACTION_LABEL[r.action]}</span>
        </span>
      ),
    },
    {
      ...
      render: (r) => (
        <span className="min-w-0">
          <Link href={`/phase1/admin/agents/${r.subjectId}`} className="block truncate text-[14px] font-medium text-p1-text hover:underline underline-offset-4">{r.subjectName}</Link>
          {r.listingRef && <span className="font-mono text-[12.5px] text-p1-text-3">{r.listingRef}</span>}
        </span>
      ),
    },
    { key: 'actor', header: 'Officer', hideBelow: 'md', render: (r) => <span className="truncate text-[13px] text-p1-text-2">{r.actorEmail}</span> },
    {
      key: 'reason', header: 'Reason', hideBelow: 'lg',
      render: (r) => <span className="text-[13px] leading-5 text-p1-text-2">{r.reason ?? '—'}</span>,
    },
  ];
  ...
  return (
    <>
      ...
      <Callout tone="info" className="mb-6" title="The funnel and plan mix below are sample figures">
        The audit trail at the foot of this page is real — every row is a decision an officer actually made on this
        instance. The onboarding funnel and plan mix still come from fixed data.
      </Callout>
      ...
      <SectionCard
        ...
        description="Every verification, moderation and suspension decision, with the officer who made it."
...
function AuditTable({ columns, rows }: { columns: Column<AuditRow>[]; rows: AuditRow[] }) {
  ...
  return (
    <div ref={ref} className={cx(seen && 'vr-stagger')}>
      <DataTable columns={columns} rows={rows} rowKey={(r) => r.id} caption="Audit trail" empty={<EmptyState compact title="Nothing recorded yet" description="Verification, moderation and suspension decisions are written here as they are made." />} />
    </div>
  );
}
